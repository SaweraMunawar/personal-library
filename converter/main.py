...
files = st.file_uploader("Upload on Excel Files", type=["csv", "xlsx"], accept_multiple_files=True)
...
if files:
    for file in files:
        ...
        if st.button(f"⬇️ Download {file.name} as {format_choice}"):
            ...
            output.seek(0)
            st.download-button("⬇️ Download File", file_name=new_name, data=output, mime=mime)
